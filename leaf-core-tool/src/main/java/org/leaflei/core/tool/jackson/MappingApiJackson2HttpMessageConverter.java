...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	 ...
	 */
	public MappingApiJackson2HttpMessageConverter(ObjectMapper objectMapper, LeafJacksonProperties properties) {
		super(objectMapper, initWriteObjectMapper(objectMapper), initMediaType(properties));
	}

	private static List<MediaType> initMediaType(LeafJacksonProperties properties) {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper) {
		...
		// null 处理
		writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new LeafBeanSerializerModifier()));
		writeObjectMapper.getSerializerProvider().setNullValueSerializer(LeafBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
